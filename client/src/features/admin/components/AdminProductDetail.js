...
export default function AdminProductDetail() {
  ...
  const handleCart = (e) => {
    e.preventDefault();
    if (product.stock <= 0) {
      alert.error('This product is out of stock', { position: 'top center' });
      return;
    }
    if (items.findIndex((item) => item.product.id === product.id) < 0) {
      ...
      alert.success('Item added to Cart', { position: 'top center' });
    } else {
      alert.error('Item Already added', { position: 'top center' });
    }
  };
