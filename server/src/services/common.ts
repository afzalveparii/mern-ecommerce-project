...
class CommonService {
  ...
  constructor() {
    this.transporter = nodemailer.createTransport({
      ...
      auth: {
        user: 'afajal1432@gmail.com', // gmail
        pass: process.env.MAIL_PASSWORD, // pass
      },
    });
  }

  isAuth(req: Request, res: Response, done: NextFunction): any {
    return passport.authenticate('jwt', { session: false })(req, res, done);  
    
  }

  sanitizeUser(user: User): { id: string; role: string } {
    return { id: user.id, role: user.role };
  }

  cookieExtractor(req: Request): string | null {
    let token = null;
    if (req && req.cookies) {
  ...
  async sendMail(options: MailOptions): Promise<any> {
    let info = await this.transporter.sendMail({
      from: '"E-commerce" <afajal1432@gmail.com>', // sender address
      to: options.to,
  ...
  invoiceTemplate(order: Order): string {
    // The HTML template remains the same, just wrapped in a method
    return `<!DOCTYPE html>
     <html>
  <head>
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
            ...
            <tr>
              <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
                <p style="margin: 0;">Here is a summary of your recent order. If you have any questions or concerns about your order, please <a href="afajal1432@gmail.com">contact us</a>.</p>
              </td>
            </tr>
            <!-- end copy -->
              ...
              <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                  ...
                  ${order.items.map(item => `<tr>
                    ...
                    <td align="left" width="20%" style="padding: 6px 12px;font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">$${(item.product.price * (1 - item.product.discountPercentage / 100)).toFixed(2)}</td>
                  </tr>`)
